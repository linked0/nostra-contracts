...
/**
 * Simulate continuous buying pressure on Ohtani YES tokens
 *
 * This script executes 6 trades with steady price increases to demonstrate
 * sustained market demand and upward price movement.
 *
 * Price progression: $0.52 → $0.54 → $0.56 → $0.58 → $0.60 → $0.62
 * Pattern: Steady accumulation with +$0.02 incremental increases
 *
 * Usage:
 ...
 *   - Need ~60 YES tokens (6 trades × 10 tokens each)
 */
...
async function main() {
  ...
  console.log(`\n📈 Simulating Continuous Buying Pressure...`);
  console.log(`Pattern: Steady accumulation (+$0.02 per trade)`);
  ...
  // Simulate 6 trades with steady accumulation pattern
  // Continuous buying pressure: +$0.02 per trade showing sustained demand
  const trades = [
    { price: 0.52, description: "Initial accumulation (YES at $0.52)" },
    { price: 0.54, description: "Steady buying interest (YES at $0.54)" },
    { price: 0.56, description: "Continued demand (YES at $0.56)" },
    { price: 0.58, description: "Growing confidence (YES at $0.58)" },
    { price: 0.60, description: "Strong momentum (YES at $0.60)" },
    { price: 0.62, description: "Peak buying pressure (YES at $0.62)" },
  ];

  for (let i = 0; i < trades.length; i++) {
    const trade = trades[i];
    console.log(`\n${"═".repeat(70)}`);
    console.log(`💹 Trade ${i + 1}/6: ${trade.description}`);
  ...
  console.log(`\n${"═".repeat(70)}`);
  console.log(`🎉 BUYING PRESSURE SIMULATION COMPLETE`);
  console.log(`${"═".repeat(70)}`);
  console.log(`\n📊 Price Movement: $0.52 → $0.62 (+19.2% increase)`);
  console.log(`💡 Next steps:`);
  console.log(`1. Check prices: yarn market:prices`);
  console.log(`2. Final price should show YES at $0.62 (62% probability)`);
}
