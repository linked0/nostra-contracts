...
/**
 * Place bets on Vladimir Guerrero Jr. winning World Series MVP
 *
 * Usage:
 * 1. Add to .env:
 *    VLADIMIR_GUERRERO_JR_CONDITION_ID="0x..."
 *    VLADIMIR_GUERRERO_JR_YES_BET="0"  # Amount for YES (0 to skip)
 *    VLADIMIR_GUERRERO_JR_NO_BET="50"  # Amount for NO (0 to skip)
 *
 * 2. Run:
 ...
 * Examples:
 *    # Bet 100 USDC on YES
 *    VLADIMIR_GUERRERO_JR_YES_BET=100 VLADIMIR_GUERRERO_JR_NO_BET=0
 *
 *    # Bet 50 USDC on NO (hedge)
 *    VLADIMIR_GUERRERO_JR_YES_BET=0 VLADIMIR_GUERRERO_JR_NO_BET=50
 *
 *    # Bet on both sides
 *    VLADIMIR_GUERRERO_JR_YES_BET=100 VLADIMIR_GUERRERO_JR_NO_BET=50
 */
...
async function main() {
  ...
  console.log(`\n💰 Placing bets on Vladimir Guerrero Jr. MVP Market...`);
  console.log(`Network: ${networkName}`);
  console.log(`Chain ID: ${network.chainId}`);

  // Get parameters from environment
  const conditionId = process.env.VLADIMIR_GUERRERO_JR_CONDITION_ID;
  const yesBetAmount = parseInt(process.env.VLADIMIR_GUERRERO_JR_YES_BET || "0");
  const noBetAmount = parseInt(process.env.VLADIMIR_GUERRERO_JR_NO_BET || "0");

  if (!conditionId) {
    throw new Error("VLADIMIR_GUERRERO_JR_CONDITION_ID environment variable is required");
  }

  if (yesBetAmount === 0 && noBetAmount === 0) {
    console.log(`\n⚠️  No bets configured. Set VLADIMIR_GUERRERO_JR_YES_BET and/or VLADIMIR_GUERRERO_JR_NO_BET in .env`);
    return;
  }

  console.log(`\n📊 Bet Configuration:`);
  console.log(`Player: Vladimir Guerrero Jr.`);
  ...
  console.log(`\n${"═".repeat(70)}`);
  console.log(`🎉 BETTING COMPLETE - Vladimir Guerrero Jr.`);
  console.log(`${"═".repeat(70)}`);
  ...
  console.log(`\n💡 Trading Strategy:`);
  if (yesBetAmount > 0 && noBetAmount === 0) {
    console.log(`  You bet on YES (Guerrero wins)`);
    ...
    console.log(`  You bet on NO (Guerrero doesn't win)`);
