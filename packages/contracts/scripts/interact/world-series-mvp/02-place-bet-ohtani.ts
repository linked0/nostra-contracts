...
/**
 * Place bets on Shohei Ohtani winning World Series MVP
 *
 * Usage:
 * 1. Add to .env:
 *    SHOHEI_OHTANI_CONDITION_ID="0x..."
 *    SHOHEI_OHTANI_YES_BET="100"  # Amount for YES (0 to skip)
 *    SHOHEI_OHTANI_NO_BET="0"     # Amount for NO (0 to skip)
 *
 * 2. Run:
 ...
 * Examples:
 *    # Bet 100 USDC on YES
 *    SHOHEI_OHTANI_YES_BET=100 SHOHEI_OHTANI_NO_BET=0
 *
 *    # Bet 50 USDC on NO (hedge)
 *    SHOHEI_OHTANI_YES_BET=0 SHOHEI_OHTANI_NO_BET=50
 *
 *    # Bet on both sides (for liquidity or arbitrage)
 *    SHOHEI_OHTANI_YES_BET=100 SHOHEI_OHTANI_NO_BET=50
 */
...
async function main() {
  ...
  console.log(`\n💰 Placing bets on Shohei Ohtani MVP Market...`);
  console.log(`Network: ${networkName}`);
  console.log(`Chain ID: ${network.chainId}`);

  // Get parameters from environment
  const conditionId = process.env.SHOHEI_OHTANI_CONDITION_ID;
  const yesBetAmount = parseInt(process.env.SHOHEI_OHTANI_YES_BET || "0");
  const noBetAmount = parseInt(process.env.SHOHEI_OHTANI_NO_BET || "0");

  if (!conditionId) {
    throw new Error("SHOHEI_OHTANI_CONDITION_ID environment variable is required");
  }

  if (yesBetAmount === 0 && noBetAmount === 0) {
    console.log(`\n⚠️  No bets configured. Set SHOHEI_OHTANI_YES_BET and/or SHOHEI_OHTANI_NO_BET in .env`);
    return;
  }

  console.log(`\n📊 Bet Configuration:`);
  console.log(`Player: Shohei Ohtani`);
  ...
  console.log(`\n${"═".repeat(70)}`);
  console.log(`🎉 BETTING COMPLETE - Shohei Ohtani`);
  console.log(`${"═".repeat(70)}`);
  ...
  console.log(`\n💡 Trading Strategy:`);
  if (yesBetAmount > 0 && noBetAmount === 0) {
    console.log(`  You bet on YES (Ohtani wins)`);
    ...
    console.log(`  You bet on NO (Ohtani doesn't win)`);
